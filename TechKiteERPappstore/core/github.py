...
class GitHubClient:
    ...
    def get_tags(self, page: int, size: int = 100):
        url = '%s/repos/TechKiteERP/server/tags' % self.base_url
...
def sync_releases(versions: Iterable[str]) -> None:
    ...
    :return:
    """
    current_releases = TechKiteERPRelease.objects.all()
    imported_releases = [
        TechKiteERPRelease.objects.get_or_create(version=version)[0]
    ...
    if imported_releases:
        ...
        for release in get_old_releases(current_releases, imported_releases):
            ...
            release.save()
        # set latest release
        TechKiteERPRelease.objects.update(is_current=False)
        latest = max(imported_releases, key=lambda v: Version(v.version))
        latest.is_current = True
        latest.save()


TechKiteERPReleases = List[TechKiteERPRelease]


def get_old_releases(current: TechKiteERPReleases,
                     imported: TechKiteERPReleases) -> TechKiteERPReleases:
