...
from TechKiteERPappstore.core.facades import distinct
from TechKiteERPappstore.core.rating import compute_rating
from TechKiteERPappstore.core.versioning import pad_min_version, \
...
class App(TranslatableModel):
    ...
    def _get_grouped_releases(self, get_release_func):
        releases = TechKiteERPRelease.objects.all()
...
class TechKiteERPReleaseManager(Manager):
    def get_current(self):
        return self.get_queryset().filter(is_current=True)[:1]


class TechKiteERPRelease(Model):
    objects = TechKiteERPReleaseManager()
    version = CharField(max_length=100, verbose_name=_('TechKiteERP version'),
    ...
    has_release = BooleanField(verbose_name=_('Has a release'),
                               help_text=_(
                                   ...
                                   'TechKiteERP version that can be '
    ...
    class Meta:
        verbose_name = _('TechKiteERP release')
        verbose_name_plural = _('TechKiteERP releases')
        ordering = ('-version',)

    @staticmethod
    def get_current_main():
        current = TechKiteERPRelease.objects.get_current()
        if len(current) > 0:
