...
class SyncTechKiteERPReleasesTest(TestCase):

    @patch.object(GitHubClient, 'get_tags')
    def test_sync(self, get_tags):
        get_tags.side_effect = self._get_tags
        call_command('syncTechKiteERPreleases', '--oldest-supported=11.0.0',
                     stdout=StringIO())

        latest = TechKiteERPRelease.objects.get(version='12.0.5')
    ...
    def test_sync_print(self, get_tags):
        ...
        self.assertEquals(0, TechKiteERPRelease.objects.count())
