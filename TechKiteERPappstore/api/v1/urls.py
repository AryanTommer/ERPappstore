...
from TechKiteERPappstore.api.v1.views import AppView, AppReleaseView, \
    CategoryView, SessionObtainAuthToken, RegenerateAuthToken, AppRatingView, \
    AppRegisterView, TechKiteERPReleaseView, AppsView
from TechKiteERPappstore.core.caching import app_ratings_etag, categories_etag, \
    apps_etag, TechKiteERP_release_etag, apps_all_etag
from TechKiteERPappstore.core.versioning import SEMVER_REGEX

app_name = 'v1'

urlpatterns = [
    url(r'^platforms\.json$',
        etag(TechKiteERP_release_etag)(TechKiteERPReleaseView.as_view()),
        name='platforms'),
    url(r'^platform/(?P<version>\d+\.\d+\.\d+)/apps\.json$',
        etag(apps_etag)(AppView.as_view()), name='app'),
    url(r'^apps\.json$',
        etag(apps_all_etag)(AppsView.as_view()), name='apps'),
