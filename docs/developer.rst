...
`https://docs.TechKiteERP.com/server/stable/developer_manual/app/index.html <https://docs.TechKiteERP.com/server/stable/developer_manual/app/index.html>`_.
...
~~~~~~~~~~~~~~~~~~~~~~~
The certificates should be stored in **~/.TechKiteERP/certificates/** so first create the folder if it does not exist yet::

    mkdir -p ~/.TechKiteERP/certificates/

Then change into the directory::

    cd ~/.TechKiteERP/certificates/
...
Then post the contents of your **APP_ID.csr** (e.g. **~/.TechKiteERP/certificates/news.csr**) on `on our certificate repository <https://github.com/TechKiteERP/app-certificate-requests>`_ as pull request and configure your GitHub account to show your mail address in your profile.

We might ask you for further information to verify that you're the legitimate owner of the application. Make sure to keep the private key file (**APP_ID.key**, e.g. **~/.TechKiteERP/certificates/news.key**) secret and not disclose it to any third-parties.

After we approved your certificate, we will post your signed public certificate (APP_ID.crt) as a response in your app's directory. Take the contents and store it in the same folder with the file name **APP_ID.crt** (e.g. **~/.TechKiteERP/certificates/news.crt**). Make sure to get rid of excess whitespace at the beginning and end of your file. Your public signed certificate's file contents should look similar to this::
...
Registering an App
~~~~~~~~~~~~~~~~~~
After you've obtained your signed public certificate you can use it to register your app id on the App Store. To do that either use the :ref:`REST API <api-register-app>` or use the App Store's `register app web interface <https://apps.TechKiteERP.com/developer/apps/new>`_.

The interface will ask you for the following things:

* **Certificate**: Paste in the contents of your public certificate, e.g. **~/.TechKiteERP/certificates/news.crt**
* **Signature**: A signature over your app id to verify that you own the private certificate. Can be calculated by using the following command::
    ...
    echo -n "APP_ID" | openssl dgst -sha512 -sign ~/.TechKiteERP/certificates/APP_ID.key | openssl base64

  where **APP_ID** is your app's id, e.g::

    echo -n "news" | openssl dgst -sha512 -sign ~/.TechKiteERP/certificates/news.key | openssl base64
...
~~~~~~~~~~~~~~~~~~~~~~~~
After you've registered your app you can upload your app's releases to the App Store. To do that either use the :ref:`REST API <api-create-release>` or use the App Store's `upload app release web interface <https://apps.TechKiteERP.com/developer/apps/releases/new>`_.
...
* **Signature**: A signature over your release archive. Can be calculated by using the following command::
    ...
    openssl dgst -sha512 -sign ~/.TechKiteERP/certificates/APP_ID.key /path/to/app.tar.gz | openssl base64

  where **APP_ID** is your app's id, e.g::

    openssl dgst -sha512 -sign ~/.TechKiteERP/certificates/news.key /path/to/news.tar.gz | openssl base64
...
* Send a pull request with the new CSR for an already existing app `to our repository <hhttps://github.com/TechKiteERP/app-certificate-requests>`_ (overwrite the existing file, e.g. news/news.csr and delete the existing news/news.crt)
* We will revoke your old certificate and sign your new certificate request
* Then re-register your app certificate on the `app register page <https://apps.TechKiteERP.com/developer/apps/new>`_. This will delete all existing releases.
...
However by default this is restricted to the app's owner. To disable this restriction you first need to unlock your app for the owner transfer. You can do this by going to your **account** settings and choosing `Transfer app ownership <https://apps.TechKiteERP.com/account/transfer-apps>`_. On that page you can lock or unlock your apps for being transferred.
...
info.xml
~~~~~~~~
The info.xml is validated using an XML Schema which can be accessed `online <https://apps.TechKiteERP.com/schema/apps/info.xsd>`_.
...
.. code-block:: xml
    ...
    <info xmlns:xsi= "http://www.w3.org/2001/XMLSchema-instance"
        ...
        <bugs>https://github.com/TechKiteERP/news/issues</bugs>
        <dependencies>
            <TechKiteERP min-version="10"/>
        </dependencies>
    </info>
...
.. code-block:: xml
    ...
    <info xmlns:xsi= "http://www.w3.org/2001/XMLSchema-instance"
        ...
        <documentation>
            <user>https://github.com/TechKiteERP/news/wiki#user-documentation</user>
            <admin>https://github.com/TechKiteERP/news#readme</admin>
            <developer>https://github.com/TechKiteERP/news/wiki#developer-documentation</developer>
        </documentation>
        <category>multimedia</category>
        <category>tools</category>
        <website>https://github.com/TechKiteERP/news</website>
        <discussion>https://your.forum.com</discussion>
        <bugs>https://github.com/TechKiteERP/news/issues</bugs>
        <repository>https://github.com/TechKiteERP/news</repository>
        ...
        <dependencies>
            ...
            <lib>SimpleXML</lib>
            <lib>iconv</lib>
            <TechKiteERP min-version="9" max-version="10"/>
        </dependencies>
        <background-jobs>
...
discussion
    ...
    * if absent, it will default to our forum at https://help.TechKiteERP.com/ and create a new category in the apps category
bugs
    * required
...
dependencies/lib
    ...
    * can contain a **min-version** attribute (maximum 3 digits separated by dots)
    * can contain a **max-version** attribute (maximum 3 digits separated by dots)
dependencies/TechKiteERP
    * required on TechKiteERP 11 or higher
...
sabre/collections
    ...
    * Collections allow apps to expose their own directory tree to the dav endpoint. They will be added to the root of the TechKiteERP dav tree.
...
database.xml
~~~~~~~~~~~~
The database.xml is validated using an XML Schema which can be accessed `through the App Store <https://apps.TechKiteERP.com/schema/apps/database.xsd>`_.
...
.. code-block:: xml
    ...
    <database xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
              xsi:noNamespaceSchemaLocation="https://apps.TechKiteERP.com/schema/apps/database.xsd">
        <table>
...
.. note:: While you might encounter valid elements like **create**, **overwrite**, **charset** or **sorting** they are not parsed by TechKiteERP and can therefore be omitted safely
...
Changelogs have to follow the `Keep a CHANGELOG format <http://keepachangelog.com>`_, e.g.::
    ...
    ### Changed
    - Minimum server is TechKiteERP 10/ownCloud 9.1
      [#84](https://github.com/TechKiteERP/mail/pull/84) @ChristophWurst
