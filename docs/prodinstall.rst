...
Afterwards change into your preferred target folder, clone the repository using git and change into it::
    ...
    cd /path/to/target
    git clone https://github.com/TechKiteERP/appstore.git
    cd appstore
...
--------------------------
To get your instance running in production you need to create your production settings file in **TechKiteERPappstore/settings/production.py** which overwrites and enhances the settings defined in **TechKiteERPappstore/settings/base.py**. The production settings file is excluded from version control. For a basic configuration take a look at :ref:`an example production configuration <production-configuration>`

Then set the file as the active settings file::

    export DJANGO_SETTINGS_MODULE=TechKiteERPappstore.settings.production
...
This will place the contents inside the folder configured under the key **STATIC_ROOT** and **MEDIA_ROOT** inside your **TechKiteERPappstore/settings/production.py**
...
.. code-block:: apache
    ...
    WSGIScriptAlias / /path/to/appstore/TechKiteERPappstore/wsgi.py
    WSGIPassAuthorization On
    Alias /static/ /var/www/production-domain.com/static/
    Alias /schema/apps/info.xsd /path/to/appstore/TechKiteERPappstore/api/v1/release/info.xsd
    Alias /schema/apps/database.xsd /path/to/appstore/TechKiteERPappstore/api/v1/release/database.xsd

    <Directory /path/to/appstore/TechKiteERPappstore>
        <Files wsgi.py>
            Require all granted
        </Files>
    </Directory>

    <Directory /path/to/appstore/TechKiteERPappstore/api/v1/release>
        <Files info.xsd>
            Require all granted
...
Afterwards your **client id** and **client secret** are displayed. These need to be saved inside the database. To do that, either log into the admin interface, change your site's domain and add GitHub as a new social application or run the following command::
    ...
    python manage.py setupsocial --github-client-id "CLIENT_ID" --github-secret "SECRET" --domain apps.TechKiteERP.com
...
After obtaining the token from GitHub, add it anywhere in your settings file (**TechKiteERPappstore/settings/production.py**), e.g.:
...
You can run the command by giving it the oldest supported TechKiteERP version::
     ...
     python manage.py syncTechKiteERPreleases --oldest-supported="12.0.0"
...
You can also do a test run and see what kind of versions would be imported::
     ...
     python manage.py syncTechKiteERPreleases --oldest-supported="12.0.0" --print
...
::
    ...
    venv/bin/python manage.py syncTechKiteERPreleases --oldest-supported="12.0.0"
