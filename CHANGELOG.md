...
- Removed Support for TechKiteERP 11
...
- Add support for generating apps for TechKiteERP 13
- Add support for personal setting tags in info.xml and ids instead of URLs for documentation
- Add another flag to differentiate between supported and unreleased TechKiteERP releases

## [3.2.0]  - 2018-02-04

### Added

- **syncTechKiteERPreleases** command to sync TechKiteERP releases directly from GitHub

### Removed

- TechKiteERP releases are not imported via fixtures anymore, use the **syncTechKiteERPreleases** command
...
- **API (v1)**: Add a new API route to fetch all TechKiteERP releases

### Changed

- Bring back **discussion** tag in info.xml which allows you to define your own forum link. If absent, it will default to our forum at help.TechKiteERP.com
- Use one unique button style for all buttons, #402
- Moved certificate and scaffolding to separate folders
- Moved TechKiteERP public certificate and CRL to new folder
...
- Dropped support for TechKiteERP 9 and 10:
  - owncloud tag in info.xml will not be migrated to TechKiteERP tags anymore
  ...
  - v0 API was removed
  - TechKiteERPrelease.json fixtures for 9 and 10 were removed
  - TechKiteERP 10 apps can not be generated anymore
...
- Update travis configuration and fix test suite for generating TechKiteERP 12 apps
...
- Added new TechKiteERP version fixtures
...
- Added TechKiteERP 12 app template
...
- Require max-version to be present for TechKiteERP versions
...
- Add app development templates for TechKiteERP 10
...
- Manage TechKiteERP releases in the admin interface instead of a Python file
- Apps need to provide a maximum version in addition to the minimum version
- Apps do not need to provide an owncloud tag anymore for TechKiteERP 11
...
- Apps depending on TechKiteERP minor or patch versions are now rendered on the app detail and releases page
...
- Add legacy API so TechKiteERP 9 and 10 can switch to the new store
